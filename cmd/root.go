/*
Copyright © 021 NAME HERE <EMAIL ADDRESS>
...
func init() {
	...
	// rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.firestorefun.yaml)")
...
func initConfig() {
	...
	if err := viper.ReadInConfig(); err == nil {
		log.Trace().Str("config", viper.ConfigFileUsed())
	} else {
		log.Err(err).Send()
	}
}
